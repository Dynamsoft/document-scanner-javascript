 ...
 *
 * @defaultValue "https://cdn.jsdelivr.net/npm/dynamsoft-document-scanner@1.5.0/dist/document-scanner.ui.xml"
 *
 * @internal
 */
const DEFAULT_DCE_UI_PATH =
	"https://cdn.jsdelivr.net/npm/dynamsoft-document-scanner@1.5.0/dist/document-scanner.ui.xml";
...
class DocumentScanner {
	...
	private async initializeDCVResources(): Promise<void> {
		try {
			...
			this.resources.cameraEnhancer = await CameraEnhancer.createInstance(
				this.resources.cameraView,
			);
	...
	private initializeDDSConfig() {
		...
		const scannerViewConfig = {
			...
			container:
				viewContainers[EnumDDSViews.Scanner] || this.config.scannerViewConfig?.container || null,
			cameraEnhancerUIPath:
				this.config.scannerViewConfig?.cameraEnhancerUIPath || DEFAULT_DCE_UI_PATH,
		...
		const correctionViewConfig = this.showCorrectionView()
			? {
					...this.config.correctionViewConfig,
					container:
						viewContainers[EnumDDSViews.Correction] ||
						this.config.correctionViewConfig?.container ||
						null,
					templateFilePath: baseConfig.templateFilePath,
					utilizedTemplateNames: baseConfig.utilizedTemplateNames,
		...
		const resultViewConfig = this.showResultView()
			? {
					...this.config.resultViewConfig,
					container:
						viewContainers[EnumDDSViews.Result] || this.config.resultViewConfig?.container || null,
				}
			: undefined;
	 ...
	 * @internal
	 */
	private async processFileToBlob(
		file: File,
