...
export default class DocumentScannerView {
	...
	constructor(
		...
		private config: DocumentScannerViewConfig,
	) {
	...
	async initialize(): Promise<void> {
		...
		this.smartCaptureEnabled =
			(this.config?.enableSmartCaptureMode || this.config?.enableAutoCropMode) ?? false; // If autoCrop mode is enabled, smartCapture mode should be too
	...
	private async initializeElements() {
		...
		this.DCE_ELEMENTS = {
			...
			continuousScanDoneBtn: DCEContainer.shadowRoot.querySelector(
		...
		if (this.config?.showPoweredByDynamsoft === false) {
			const poweredByDynamsoft = DCEContainer.shadowRoot.querySelector(
	...
	private updateContinuousScanDoneButton() {
		...
		const textEl = this.DCE_ELEMENTS.continuousScanDoneBtn?.querySelector(
		...
		return new Promise((resolve) => {
			...
			requestAnimationFrame(() => {
				...
				const scale = Math.min(
					thumbnailRect.width / floatingRect.width,
					thumbnailRect.height / floatingRect.height,
				);
	...
	private toggleSelectCameraBox() {
		...
		const settingsBox = DCEContainer.shadowRoot.querySelector(
	...
	async toggleBoundsDetection(enabled?: boolean) {
		...
		const container = DCEContainer.shadowRoot.querySelector(
			".dce-mn-bounds-detection",
		) as HTMLElement;
		const onIcon = DCEContainer.shadowRoot.querySelector(
			".dce-mn-bounds-detection-on",
		) as HTMLElement;
		const offIcon = DCEContainer.shadowRoot.querySelector(
	...
	async openCamera(): Promise<void> {
		try {
			...
			if (this.DCE_ELEMENTS.continuousScanDoneBtn) {
				if (
					this.resources.enableContinuousScanning &&
					(this.resources.completedScansCount ?? 0) > 0
				) {
					this.DCE_ELEMENTS.continuousScanDoneBtn.style.display = "block";
					this.updateContinuousScanDoneButton();
				} else {
	...
	async takePhoto() {
		...
		try {
			...
			const shouldUseLatestFrame =
				!this.boundsDetectionEnabled ||
				(this.boundsDetectionEnabled && this.capturedResultItems?.length <= 1);
			...
			correctedImageResult = await this.normalizeImage(
				detectedQuadrilateral.points,
				originalImageData,
			);
	...
	private async handleAutoCaptureMode(result: CapturedResult) {
		...
		if (
			(result.processedDocumentResult?.detectedQuadResultItems?.[0] as any)
				?.crossVerificationStatus === 1
		)
